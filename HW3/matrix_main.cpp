...
void StudentTest(){
    ...
    //Set & get
    double d1;
    //Testing long decimals
    Matrix m6(10,1,12);
    assert(m6.set(0,0,0.1111));
    assert(m6.set(9,0,11.5999999));
    assert(m6.get(0,0,d1));
    assert(d1==0.1111);
    assert(m6.get(9,0,d1));
    assert(d1==11.5999999);

    //Addition
    //Adding empty matrix
    std::cout << "Adding empty matrix" << std::endl;
    Matrix m5;
    Matrix m7;
    m7.add(m5);
    std::cout << "The result:" << std::endl << m7;
    assert(m5 == m7);

    //Adding matrix to itself
    std::cout << "Adding matrix to itself" << std::endl;
    assert(m6.set(3,0,0.43));
    assert(m6.set(5,0,12000));
    m6.add(m6);
    std::cout << "This results:" << std::endl << m6;
    //testing get functions
    double* r1 = NULL;
    double* r2 = NULL;
    r1 = m6.get_row(3);
    r2 = m6.get_row(5);
    assert(r1[0] == 0.86 && r2[0] == 24000);

    delete [] r1;
    delete [] r2;

    //Subtraction
    //Subtracting empty matrix
    std::cout << "Adding empty matrix" << std::endl;
    m7.subtract(m5);
    std::cout << "The result:" << std::endl << m7;
    assert(m5 == m7);
    ...
    m8.set(1,0,2);
    m8.set(1,1,2);
    std::cout << "Testing swap row function" << std::endl << "Original:" << m8;
    m8.swap_row(0,1);
    std::cout << "Final:" << m8;

    r1 = m8.get_row(0);
    r2 = m8.get_row(1);
    assert(r1[0] == 2 && r2[0] == 0);

    delete [] r1;
    delete [] r2;

    //Testing multiply by coefficient
    m8.multiply_by_coefficient(10);

    std::cout << "Multiplying by coefficient of 10" << m8;

    r1 = m8.get_row(0);
    r2 = m8.get_row(1);
    assert(r1[0] == 20 && r2[0] == 0);
    ...
    std::cout << "Testing transpose" << std::endl << "Testing with empty matrix";
    m5.transpose();
    std::cout << m5;
    assert((m5.num_cols() == NULL) && (m5.num_rows() == NULL));

    //testing quarter function
    //Normal matrix with even numbers as rows and columns
    std::cout << std::endl << "Testing quarter function:" << std::endl << "Original Matrix:" << std::endl;
    Matrix m12(4,4,8);
    m12.set(0,1,2);
    m12.set(0,2,2.6);
    m12.set(0,3,4.5);
    ...
    assert(double_compare(comparison_value,8));
    assert(ma1[0].get(1,1,comparison_value));
    assert(double_compare(comparison_value,8.9));

    //Upper Right
    assert(ma1[1].get(0,0,comparison_value));
    assert(double_compare(comparison_value,2.6));
    assert(ma1[1].get(1,1,comparison_value));
    assert(double_compare(comparison_value,8));
    ...
    //Testing odd number of rows and columns
    Matrix m13(7,7,0);
    m13.set(0,1,2);
    m13.set(0,4,2.6);
    m13.set(0,6,4.5);
    ...
    std::cout << "Odd number of rows and columns:" << std::endl << m13 << std::endl;
    Matrix* ma2 = NULL;
    ma2 = m13.quarter();
    assert(ma2 != NULL);
    ...
    for(unsigned int i=0; i<4; i++){
        assert((ma2[i].num_rows() == 4) && (ma2[i].num_cols() == 4));
    }

    //Upper Left
    assert(ma2[0].get(0,0,comparison_value));
    assert(double_compare(comparison_value,0));
    assert(ma2[0].get(3,3,comparison_value));
    assert(double_compare(comparison_value,0));

    //Upper Right
    assert(ma2[1].get(0,0,comparison_value));
    assert(double_compare(comparison_value,0));
    assert(ma2[1].get(3,3,comparison_value));
    assert(double_compare(comparison_value,4.9));

    //Lower Left
    assert(ma2[2].get(0,0,comparison_value));
    assert(double_compare(comparison_value,0));
    assert(ma2[2].get(3,3,comparison_value));
    assert(double_compare(comparison_value,4.9));

    //Lower Right
    assert(ma2[3].get(0,0,comparison_value));
    assert(double_compare(comparison_value,0));
    assert(ma2[3].get(3,3,comparison_value));
    assert(double_compare(comparison_value,9.2));
    ...
    std::cout << "One column with odd number of rows:" << std::endl << m14 << std::endl;
    Matrix* ma3 = NULL;
    ma3 = m14.quarter();
    assert(ma2 != NULL);
    ...
    for(unsigned int i=0; i<4; i++){
        assert((ma3[i].num_rows() == 3) && (ma3[i].num_cols() == 1));
    }
    assert((ma3[0] == ma3[1]) && (ma3[2] == ma3[3]));

    //Upper Left
    assert(ma3[0].get(0,0,comparison_value));
    assert(double_compare(comparison_value,2));
    assert(ma3[0].get(2,0,comparison_value));
    assert(double_compare(comparison_value,4.5));
    ...
    //Lower Left
    assert(ma3[2].get(0,0,comparison_value));
    assert(double_compare(comparison_value,4.5));
    assert(ma3[2].get(2,0,comparison_value));
    assert(double_compare(comparison_value,8.9));
